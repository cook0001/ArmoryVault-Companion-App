...
echo "Building Nightly Release APK (v$VERSION)..."
...
echo "Publishing APK to GitHub Prerelease v$VERSION..."
...
if [ -s "$NOTES_FILE" ] && [ "$(awk 'NF' "$NOTES_FILE" | wc -l | tr -d ' ')" -gt 0 ]; then
    ...
    gh release create "v$VERSION" "$APK_PATH" \
        --prerelease \
        --title "Nightly Release v$VERSION" \
...
echo "✅ Successfully published Nightly Release v$VERSION to GitHub!"
